...
class SelectCategories extends StatefulWidget {
  ...
  SelectCategories({Key? key, required this.product}) : super(key: key);
      ...
      body: ScopedModelDescendant<AppStateModel>(
        builder: (context, child, model) {
          if (model.categories.length > 0) {
            return CategoryList(
                categories: model.categories, product: widget.product);
          }
...
class CategoryList extends StatefulWidget {
  ...
  final VendorProduct product;
  CategoryList({Key? key, required this.categories, required this.product})
      : super(key: key);
  @override
  ...
  Widget _buildTiles(Category category) {
    ...
    if (subCategories.isEmpty) {
      ...
      return ExpansionTile2(
        ...
        //subtitle: category.description.isEmpty || category.description == null ? null : Text(parseHtmlString(category.description), maxLines: 1),
  ...
  Container leadingIcon(Category category) {
    return Container(
      ...
      child: CachedNetworkImage(
        imageUrl: category.image.src.isNotEmpty ? category.image.src : '',
